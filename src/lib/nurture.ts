 ...
 */
export function calculateIsDue(lead: {
  status: string;
  current_step: number;
  created_at: string;
  last_sent_at: string | null;
  metadata?: { lead_status?: string };
}): boolean {
  ...
  const baseline =
    lead.current_step === 0 || !lead.last_sent_at
...
export function getDaysUntilDue(lead: {
  current_step: number;
  created_at: string;
  last_sent_at: string | null;
}): number {
  ...
  const daysElapsed = Math.ceil(
    (Date.now() - baseline.getTime()) / (1000 * 60 * 60 * 24),
  );
...
export function personalizeMessage(
  ...
  if (nickname && nickname.trim()) {
    ...
    nameToUse = titleRegex.test(trimmedNick)
      ? trimmedNick
      : `Dr. ${trimmedNick}`;
  } else {
  ...
  return (
    text
      ...
      .replace(
        /\[CLINIC_TYPE\]/g,
        clinicType ? clinicType.toLowerCase() : "clinic",
      )
  );
}
...
export const APPOINTMENT_CONFIRMATIONS: AppointmentConfirmation[] = [
  {
    ...
    buildMessage: (leadName, appointmentTime, nickname) => {
      ...
      if (nickname && nickname.trim()) {
        const trimmedNick = nickname.trim();
        name = titleRegex.test(trimmedNick)
          ? trimmedNick
          : `Dr. ${trimmedNick}`;
      } else {
...
export function parseAppointmentInfo(text: string): AppointmentInfo | null {
  ...
  if (!match)
    return {
      dateStr: text.replace("Scheduled call for ", ""),
      title: "",
      bookerEmail: "",
    };
 ...
 */
export function getNextDueTimestamp(lead: {
  current_step: number;
  created_at: string;
  last_sent_at: string | null;
}): number {
