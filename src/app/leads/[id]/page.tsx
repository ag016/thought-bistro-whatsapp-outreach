  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-color)', color: 'var(--text-color)', paddingBottom: 60 }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--bg-color)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border-color)', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={() => router.push('/')} style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: 10, color: 'var(--accent-color)', padding: '8px 14px', cursor: 'pointer', fontSize: 18, fontWeight: 700, transition: 'all 0.2s ease' }}>←</button>
        <div>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={handlePause} style={{ padding: '8px 14px', borderRadius: 10, border: '1px solid var(--border-color)', background: 'var(--surface-color)', color: lead.status === 'paused' ? 'var(--accent-color)' : 'var(--text-color)', opacity: lead.status === 'paused' ? 1 : 0.6, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
      ...
      <div style={{ padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 20 }}>
          <button onClick={() => setLeftCollapsed(!leftCollapsed)} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border-color)', background: leftCollapsed ? 'var(--accent-color)' : 'var(--surface-color)', color: leftCollapsed ? 'var(--bg-color)' : 'var(--text-color)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            {leftCollapsed ? 'Show Info' : 'Hide Info'}
          </button>
          <button onClick={() => setRightCollapsed(!rightCollapsed)} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border-color)', background: rightCollapsed ? 'var(--accent-color)' : 'var(--surface-color)', color: rightCollapsed ? 'var(--bg-color)' : 'var(--text-color)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            {rightCollapsed ? 'Show Actions' : 'Hide Actions'}
          </button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: leftCollapsed ? '0px' : '300px', gap: '24px' }}>
          {!leftCollapsed && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: 18, padding: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent-color)', marginBottom: 14 }}>NURTURE PROGRESS</div>
                <div style={{ display: 'flex', gap: 3, marginBottom: 10 }}>
                  {NURTURE_SEQUENCE.map((_, i) => (
                    <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i < lead.current_step ? 'var(--accent-color)' : 'var(--border-color)' }} />
                  ))}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                  <span style={{ fontWeight: 700 }}>Step {Math.min(lead.current_step + 1, 10)} / 10</span>
                  <span style={{ color: 'var(--accent-color)' }}>{isCompleted ? 'Complete' : 'In Progress'}</span>
                </div>
              </div>

              <div style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: 18, padding: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent-color)', marginBottom: 14 }}>LEAD INFO</div>
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 6 }}>INTERNAL TAG</div>
                  <select 
                    value={lead.internal_tag || 'NEW'} 
                    onChange={e => handleUpdateTag(e.target.value)}
                    style={{ width: '100%', background: 'var(--bg-color)', color: 'var(--text-color)', border: '1px solid var(--border-color)', borderRadius: 10, padding: '10px' }}
                  >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  {m.clinic_type && <InfoField label="Clinic" value={m.clinic_type} />}
                  {m.treatment_price && <InfoField label="Price" value={m.treatment_price} />}
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: 18, padding: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent-color)', marginBottom: 16 }}>MESSAGE TIMELINE</div>
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                      <div style={{ width: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div style={{ width: 12, height: 12, borderRadius: '50%', background: sentTimestamp ? 'var(--accent-color)' : 'var(--border-color)', marginTop: 14 }} />
                        {i < NURTURE_SEQUENCE.length - 1 && <div style={{ width: 2, flex: 1, background: 'var(--border-color)', margin: '4px 0' }} />}
                      </div>
                      <div style={{ flex: 1, paddingTop: 8 }}>
                        <MessageBubble 
                          ...
                          isCurrentTarget={!sentTimestamp && i === lead.current_step}
          ...
          {!rightCollapsed && (
            <ActionCenter 
              ...
              onAddNote={async (text) => {
                ...
                const now = new Date().toISOString();
                setNotes(prev => [...prev, { lead_id: lead.id, note_text: text, created_at: now, source: 'manual' }]);
                await fetch('/api/notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ leadId: lead.id, noteText: text, createdAt: now }) });
              }}
              onDeleteNote={handleDeleteNote}
              onBookCall={async ({ callDate, eventTitle, eventNote }) => {
                if (!lead) return;
                await fetch('/api/calendar', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ leadName: lead.full_name, phone: lead.phone_number, dateStr: callDate, summary: eventTitle, description: eventNote }) });
                const now = new Date().toISOString();
                setNotes(prev => [...prev, { lead_id: lead.id, note_text: `🗓 Call: ${callDate}`, created_at: now, source: 'system' }]);
              }}
